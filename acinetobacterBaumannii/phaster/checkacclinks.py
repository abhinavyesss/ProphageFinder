...
if __name__ == '__main__':
    ...
    for ind in range(len(df)):
        ...
        try:
            ...
            if len(dnas) > 0:
                if not os.path.exists('{}phaster/intactphages/{}/'.format(os.getenv('MAIN_PATH'), df.at[ind, 'accessionNumbers'])):
                    os.mkdir('{}phaster/intactphages/{}/'.format(os.getenv('MAIN_PATH'), df.at[ind, 'accessionNumbers']))
                text = ''
                for seq in dnas:
                    text += seq + '\n'
                f = open('{}phaster/intactphages/{}.fasta'.format(os.getenv('MAIN_PATH'), df.at[ind, 'accessionNumbers']), 'w+')
                f.write(text)
                f.close()
