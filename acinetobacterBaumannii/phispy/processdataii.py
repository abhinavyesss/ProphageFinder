...
if __name__ == '__main__':
    ...
    for ind, i in enumerate(l):
        ...
        if df.at[ind, 'total'] != 0:
            ...
            cordDF = pd.read_csv('{}phispy/files/{}/prophage.tsv'.format(os.getenv('MAIN_PATH'), i), delimiter = '\t')
            tempL = []
            for j in range(len(cordDF)):
                start = cordDF.at[j, 'Start']
                stop = cordDF.at[j, 'Stop']
                tempDF = pd.read_csv('{}phispy/files/{}/prophage_information.tsv'.format(os.getenv('MAIN_PATH'), i), delimiter = '\t')
